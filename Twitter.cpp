...
//  Serial.println("nonce");
  for (i = 0; i < sizeof(nonce); i++)
    nonce[i] = (uint8_t) random(48,90);
//    Serial.print((char)nonce[i]);
}
...
Twitter::compute_authorization(const char *message)
{
  ...
  *cp++ = '&';
  ...
  auth_add_pgm(PSTR("POST&https%3A%2F%2F"));
  auth_add_pgm(server);
...
void
Twitter::compute_authorization_rt()
{
  char *cp = buffer;
...
//  auth_add_param(PSTR("oauth_consumer_key"),consumer_key,buffer);
...
Twitter::post_rt()
{
  ...
  /* Read response status line. */
//  while(http.connected())Serial.print((char)http.read());
//  Serial.print("here");
  if (!read_line(&http, buffer, buffer_len) || buffer[0] == '\0')
    {
      http.stop();
...
void
Twitter::compute_authorization_fav(String id)
{
  char *cp = buffer;
